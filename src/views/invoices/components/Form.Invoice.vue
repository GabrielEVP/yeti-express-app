...
const { handleSubmit, defineField, setValues, values, setFieldValue, meta } = useForm<Invoice>({
...
const onSubmit = handleSubmit(async (values) => {
   ...
   if (InvoiceId) {
      ...
      response = await putInvoices(values, InvoiceId);
   } else {
      errorMessage = "Error al crear la factura";
      response = await postInvoices(values);
   }
   ...
   if (!response || response.status < 200 || response.status >= 300) {
      ...
      await router.push({
         path: invoiceAppRoutes.list,
         state: {
            successMessage: InvoiceId ? "Factura actualizada correctamente" : "Factura creada correctamente",
         },
      });
   }
});
