<template>
  <ModalDetail v-if="client" :is-open="isOpen" @close="handleCloseModal">
    <div class="relative overflow-hidden">
      <ModalTitleModal>Detalle del client</ModalTitleModal>
      <div class="relative px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8">
        ...
        <div class="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 sm:p-6 border border-gray-100 dark:border-gray-700 shadow-sm">
          <div class="space-y-4 sm:space-y-5">
            <ModalContentItem>
              ...
              <template #content>{{ client.registration_number }}</template>
            </ModalContentItem>
            <ModalContentItem>
              <template #title>Tipo de client</template>
              <template #content>{{ formatClientType(client.type as ClientType) }}</template>
            </ModalContentItem>
            <ModalContentItem>
              <template #title>Credito</template>
              <template #content>{{ client.allow_credit }}</template>
            </ModalContentItem>
            <ModalContentItem>
              <template #title>Fecha de Creación</template>
              <template #content>{{ formatDateCustom(client.created_at) }}</template>
            </ModalContentItem>
            <ModalContentItem>
              <template #title>Fecha de Actualización</template>
              <template #content>{{ formatDateCustom(client.updated_at) }}</template>
            </ModalContentItem>
          </div>
        </div>
        <ModalSpecialContentItem>
          <template #title>Notas</template>
          <template #content>{{ client.notes }}</template>
        </ModalSpecialContentItem>
        <div
        ...
        >
          <div class="flex items-center gap-3 mb-4 sm:mb-6">
            ...
            <span class="font-semibold text-lg text-gray-900 dark:text-white">Emails ({{ client.emails.length }})</span>
          </div>
          <div class="space-y-3">
            <div class="space-y-2">
              <div
                v-for="(emailItem, index) in client.emails"
                :key="index"
        ...
        >
          <div class="flex items-center gap-3 mb-4 sm:mb-6">
            ...
            <span class="font-semibold text-lg text-gray-900 dark:text-white">Teléfonos ({{ client.phones.length }})</span>
          </div>
          <div class="space-y-3">
            <div class="space-y-2">
              <div
                v-for="(phoneItem, index) in client.phones"
                :key="index"
                class="group hover:bg-gray-50 dark:hover:bg-gray-700/30 rounded-xl p-3 sm:p-4 transition-all duration-200 border-b border-gray-100 dark:border-gray-700 last:border-b-0"
              >
                <p class="text-gray-800 dark:text-gray-200">
        ...
        <div
          v-if="client.addresses && client.addresses.length > 0"
          class="bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 sm:p-6 border border-gray-100 dark:border-gray-700 shadow-sm"
        >
          <div class="flex items-center gap-3 mb-4 sm:mb-6">
            <MapPin class="h-5 w-5 text-gray-600 dark:text-gray-400 flex-shrink-0" />
            <span class="font-semibold text-lg text-gray-900 dark:text-white">Direcciones ({{ client.addresses.length }})</span>
          </div>
          <div class="space-y-3">
            <div class="space-y-2">
              <div
...
import { ClientType, DetailClient, formatClientType } from '@/views/clients/models';
