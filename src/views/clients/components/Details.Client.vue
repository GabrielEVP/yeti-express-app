      ...
      <div class="space-y-4">
        ...
        <div class="grid gap-4 md:grid-cols-2 grid-cols-1">
          ...
          <ChartBilling
            ...
            title="Estadistica mensual de deliverys"
...
const lineContents = computed(() =>
  client.value?.events ? mapEventsToLineContent(client.value.events) : []
);

const now = dayjs();
const paidDeliverys = computed(() => deliveries.value.filter((i) => i.status === 'paid'));
const thisMonthDeliverys = computed(() =>
  filterByDateMonth(paidDeliverys.value, (i) => i.date, now)
);
const lastMonthDeliverys = computed(() =>
...
const pendingCountText = computed(() =>
  formatCountWithPlural(pendingCount.value, 'Delivery pendiente', 'Deliverys pendientes')
);
