<template>
   <Dropdown>
      <template #header>
         ...
         <span class="font-medium">Dirección de Recogida</span>
      </template>
      <template #data>
         <div v-if="address" class="border rounded-md p-3 bg-muted/30 dark:border-gray-700 dark:bg-gray-700">
            <div class="flex justify-between items-start">
               <div class="space-y-1 text-sm">
                  ...
                  <p><span class="font-medium">Estado:</span> {{ address.state }}</p>
                  <p><span class="font-medium">Código Postal:</span> {{ address.postalCode }}</p>
               </div>
            </div>
         </div>
