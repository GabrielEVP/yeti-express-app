...
export const billSchema = object({
  ...
  amount: number()
    .required('El monto de la factura es requerido')
...
export const serviceSchema = object({
  ...
  amount: number()
    .required('El monto del servicio es requerido')
    .min(0, 'El monto no puede ser negativo')
    .typeError('El monto debe ser un número'),
  comision: number()
    .required('La comisión del servicio es requerida')
    .min(0, 'La comisión no puede ser negativa')
    .typeError('La comisión debe ser un número'),
  bills: array(billSchema)
    .min(0, 'Debe haber al menos una factura')
    .required('Las facturas son requeridas'),
});
