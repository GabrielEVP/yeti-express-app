...
export class DebtPaymentFormAdapter {
  static fromFormFullPayment(form: any): DebtPayment {
    return new DebtPayment(form.id ?? '', 0, new Date(), form.method as PaymentMethod);
  }

  static fromFormPartialPayment(form: any): DebtPayment {
    return new DebtPayment(form.id ?? '', form.amount, new Date(), form.method as PaymentMethod);
  }
