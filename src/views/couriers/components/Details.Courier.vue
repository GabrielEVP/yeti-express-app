        ...
        <Card class="md:col-span-2 dark:bg-gray-800 dark:text-gray-100">
          <div class="p-6 space-y-5">
            <h2 class="text-xl font-semibold flex items-center gap-2 mb-4">
              <Building2 class="h-5 w-5" /> Información del Courier
            </h2>
        ...
        <div class="grid gap-4 md:grid-cols-1 lg:grid-cols-2">
          ...
          <ActivityView title="Pagos pendientes">
            ...
            <p class="text-xs text-gray-500">
              Faltan {{ unpaidDeliveriesCountThisMonth }} deliverys por pagar este mes
            </p>
          </ActivityView>
...
const unpaidDeliveriesCountThisMonth = computed(
  () =>
    deliveries.value.filter(
      (delivery) => !delivery.courierPayments?.some((p) => isSameMonthAndYear(p.date))
    ).length
);
...
const totalCommissionThisMonth = computed(() =>
  deliveries.value
    .filter((delivery) => isSameMonthAndYear(delivery.date))
    .reduce((acc, delivery) => acc + (Number(delivery.comision) || 0), 0)
);

const totalPendingThisMonth = computed(() =>
  Number((totalCommissionThisMonth.value - totalPaidThisMonth.value).toFixed(2))
);
