<template>
   <SideBar>
      <ConfirmationModal :isOpen="isOpen" message="¿Estás seguro que quieres eliminar este Courier?" @confirm="handleDeleteConfirmation" @cancel="close" />
      <Card class="p-3">
      ...
      >
         <TableRow v-for="(courier, id) in paginatedItems" :key="courier.id">
            <TableContent class="text-black dark:text-white"> {{ courier.firstName }} {{ courier.lastName }} </TableContent>
