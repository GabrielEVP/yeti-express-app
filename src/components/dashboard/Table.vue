<template>
   <Card class="mt-8">
            ...
            <TableRow>
               <TableContent v-for="(header, index) in headers" :key="index">
                  <div class="flex items-center h-12">
                     <div class="flex cursor-pointer" @click="toggleSort(header.key)">
      ...
      <div class="flex items-center justify-between px-6 py-3 border-t border-gray-200 dark:border-gray-700">
         <div class="text-xs text-gray-500 dark:text-gray-400">Mostrando {{ startIndex + 1 }} a {{ endIndex }} de {{ totalItems }} resultados</div>
...
function toggleSort(header: string) {
   if (sortConfig.value?.column === header) {
      sortConfig.value.order = sortConfig.value.order === "asc" ? "desc" : "asc";
   } else {
      sortConfig.value = { column: header, order: "desc" };
   }
